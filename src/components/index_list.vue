      ...
      <div class="">
        <div>
          <div class="kr_article_list">
            <div>
              <ul class="feed_ul">
                <IndexListItem v-for="(data,index) in items" :data="data" :key=index />
              </ul>
            </div>
          </div>
          <div class="loading_more" @click="loadMore">
            {{this.$store.state.loading_more ? "正在加载..." : "加载更多"}}
...
export default {
  ...
  methods:{
    loadMore(){
      this.$store.dispatch('loadOneMorePage',this.$store.state.cur_page)
    },
  }
}
</script>
